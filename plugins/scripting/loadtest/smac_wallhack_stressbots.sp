// ---- CHOGAN LOAD-TEST VARIANT ---------------------------------------------------------------------
// Identical to srcdslab smac_wallhack.sp (0.8.8.1) except that fake clients are NOT ignored as receivers, so
// a bot-filled server runs the OnGameFrame visibility traces for every bot pair exactly as for N human clients
// (the cheap SetTransmit part is not exercised: CS:S v92 has no sv_stressbots, bots receive no snapshots). NEVER run this variant in production.
// MISSION §5 probe 4 needs "with and without the anti-cheat wallhack module" and bots are the only clients we have.
// --------------------------------------------------------------------------------------------------
/*
...
{
    name =          "SMAC Anti-Wallhack (LOADTEST: bots treated as receivers)",
    author =        SMAC_AUTHOR,
...
void Wallhack_UpdateClientCache(int client)
{
    ...
    g_bIgnore[client] = g_bForceIgnore[client] || ((g_Game == Game_L4D || g_Game == Game_L4D2 || g_Game == Game_HIDDEN) && g_iTeam[client] != 2);
}
